...
$app->post('/review/baik/:nip', function ($nip) use ($app) 		{ (new ReviewController())->tambahReviewBaik($nip); 		})->name('reviewbaik');
$app->post('/review/buruk/:nip', function ($nip) use ($app) 		{ (new ReviewController())->tambahReviewBuruk($nip); 		})->name('reviewburuk');
...
$app->get('/matakuliah/:id', function ($id) use ($app) 	{ (new MataKuliahController())->tampilRincianMataKuliah($id); 		})->name('rincianmatkul');

$app->get('/pencarian/:query', 	function ($query) use ($app){ (new PencarianController())->tampilHasilPencarian($query); 	})->name('pencarian');
$app->post('/feedback/', 		function () use ($app)		{ (new Home())->tambahFeedback(); 						})->name('feedback');
